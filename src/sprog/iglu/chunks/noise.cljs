...
(def simplex-2d-chunk
  {:signatures '{mod289_3 ([vec3] vec3)
                 ...
                 snoise2D ([vec2] float)}
   :functions
   {'mod289_3 '([x] (- x (* (floor (/ x 289)) 289)))
    'mod289 '([x] (- x (* (floor (/ x 289)) 289)))
    'permute '([x] (mod289_3 (* x (+ 1 (* x 34)))))
    'snoise2D
    (postwalk-replace
...
(def simplex-4d-chunk
  '{:signatures {permute ([float] float)
                 ...
                 snoise4D ([vec4] float)}
    :functions
    {permute ([x] (floor (mod (* x (+ 1 (* x 34))) 289)))
     permute4 ([x] (mod (* x (+ 1 (* x 34))) 289))
